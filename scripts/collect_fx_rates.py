...
# GBP first since Patrick is UK-based; EUR/USD majors for international context
TARGET_CURRENCIES = ["GBP", "EUR", "USD", "AUD", "CAD"]
...
def collect():
    ...
    try:
        ...
        response = requests.get(
            f"{API_URL}?from=GBP&to={currencies}", timeout=10
        )
...
def write(conn, result, date):
    ...
    for currency, rate in rates.items():
        conn.execute(
            ...
            (rate_date, currency, rate, "GBP", collected_at)
        )
        records += 1
